...
Aer::Aer() {
  ...
  //  file_logger.log(tango_interface::kInfo,"My first default informational log
  //  %.02f %i",0.05,42);
}
...
//
// void Aer::accelerometer_callback(const tango_interface::AccelerometerEvent&
// event) {
...
// timeDelta,icpCountThresh, icpErrThresh, covThresh, !openLoop, iclnuim,
// reloc,photoThresh, confidence, depth, icp, fastOdom, fernThresh,
// so3,frameToFrameRGB, savefilename);
//
//处理输入帧：
//
...
//     eFusion.processFrame(rgb, depth, timestamp, currentPose,
//     weightMultiplier); delete currentPose;
//
//可选参数2:weightMultiplier (full frame fusion weight)——默认为1.f
//可以自己调整,比如针对前面跳过的帧数对参数进行调整
//
...
// void Aer::setLoggerWidthHeight_callback(const int width, const int
// height,const double fx,const double fy,const double cx,const double cy) {
