...
ElasticFusion::ElasticFusion(const int timeDelta, const int countThresh,
  ...
  LOGI("MY elasitcfusion struct init start 1 : width: %d, height: %d, myFx: %f, myFy: %f, myCx: %f, myCy: %f",
 Resolution::getInstance().width(), Resolution::getInstance().height(),
...
void ElasticFusion::createTextures() {
  ...
  textures[GPUTexture::DEPTH_FILTERED] = new GPUTexture(
      ...
      // GL_LUMINANCE16UI_EXT, GL_LUMINANCE_INTEGER_EXT, GL_UNSIGNED_SHORT,
      // false, true);
      GL_R16UI, GL_RED_INTEGER, GL_UNSIGNED_SHORT, false, true);
  check_gl_errorElasticFusion();
...
//textures[GPUTexture::RGB]->texture->Upload(rgb, GL_RGB, GL_UNSIGNED_BYTE);
  ...
  check_gl_errorElasticFusion();
  LOGI(" ElasticFusion struct Process frame Run 3");
...
//
//   check_gl_errorElasticFusion();
//   LOGI(" ElasticFusion struct Process frame test 2 1");
//
//   glBindFramebuffer(GL_FRAMEBUFFER, frame_buffer_object_);
//
//   check_gl_errorElasticFusion();
//   LOGI(" ElasticFusion struct Process frame test 2 2");
//
...
Eigen::Vector4f * ElasticFusion::savePly() {
  ...
  Eigen::Vector4f *myMapData = globalModel.downloadMap();
  check_gl_errorElasticFusion();
