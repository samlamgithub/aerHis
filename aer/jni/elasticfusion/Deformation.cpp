                            ...
                            std::vector<Constraint> * newRelativeCons)
{
    if(def.isInit())
    {
        ...
        //Target to source
        //Target to source 给之前建立的 constrain 点重新赋值 ID，constrain 点赋值的 ID 是继 pointpool 点的 ID 之后的赋值的，每次 pointpool 中的点数不同，constrain 的 ID 也也不同
            ...
            if(!fernMatch && newRelativeCons)
            {
                ...
                for(size_t i = 0; i < constraints.size(); i++)
                {
                    if(!constraints.at(i).relative && !constraints.at(i).pin)
                    {
                        ...
                        // Relative constrain 在此处产生，localdeform 在非第一次的时候，产生的 constrain 都是 Relative constrain
