...
public:
  ...
  bool addFrame(GPUTexture *imageTexture, GPUTexture *vertexTexture,
  ...
  Eigen::Matrix4f findFrame(std::vector<SurfaceConstraint> &constraints,
                            ...
                            GPUTexture *normalTexture, GPUTexture *imageTexture,
  ...
  class Frame {
  public:
    Frame(int n, int id, const Eigen::Matrix4f &pose, const int srcTime,
          const int numPixels, unsigned char *rgb = 0,
          Eigen::Vector4f *verts = 0, Eigen::Vector4f *norms = 0)
        : goodCodes(0), id(id), pose(pose), srcTime(srcTime), initRgb(rgb),
          initVerts(verts), initNorms(norms) {
