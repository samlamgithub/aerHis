...
public:
  ...
  class Constraint {
  public:
    Constraint(const Eigen::Vector3f &src, const Eigen::Vector3f &target,
               const uint64_t &srcTime, const uint64_t &targetTime,
               const bool relative, const bool pin = false)
        : src(src), target(target), srcTime(srcTime), targetTime(targetTime),
          relative(relative), pin(pin), srcPointPoolId(-1), tarPointPoolId(-1) {
    }
  ...
  void addConstraint(const Eigen::Vector4f &src, const Eigen::Vector4f &target,
                     const uint64_t &srcTime, const uint64_t &targetTime,
  ...
  bool constrain(std::vector<Ferns::Frame *> &ferns,
                 std::vector<float> &rawGraph, int time, const bool fernMatch,
