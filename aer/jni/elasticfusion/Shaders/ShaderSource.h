...
static const char combo_splatfrag_source[] =
    ...
    "    return col;}\n"
    "void main() {\n"
    "    vec3 l = normalize(vec3((vec2(gl_FragCoord) - cam.xy) / cam.zw, "
    "1.0f));\n"
    "    vec3 corrected_pos = (dot(position.xyz, normRad.xyz) / dot(l, "
    "normRad.xyz)) * l; \n"
    ...
    "    vertex = vec4((gl_FragCoord.x - cam.x) * z * (1.f / cam.z), "
    "(gl_FragCoord.y - cam.y) * z * (1.f / cam.w), z, position.w);\n"
    "    normal = normRad;\n"
    "    time = uint(colTime.z);\n"
    "    gl_FragDepth = (corrected_pos.z / (2.0 * maxDepth)) + 0.5f;\n"
    "}\n";
static const std::tuple<std::string, std::string> combo_splatfrag_tuple =
    std::make_tuple("combo_splatfrag", combo_splatfrag_source);
...
static const char copy_unstablevert_source[] =
    ...
    "       float(time) - vColor.w < float(timeDelta) && localPos.z > 0.0 && x "
    "> 0.0 && y > 0.0 && x < cols && y < rows) {\n"
    "        for(float i = x / cols - (scale * indexXStep * windowMultiplier); "
    "i < x / cols + (scale * indexXStep * windowMultiplier); i += indexXStep) "
    "{\n"
    "            for(float j = y / rows - (scale * indexYStep * "
    "windowMultiplier); j < y / rows + (scale * indexYStep * "
    "windowMultiplier); j += indexYStep) {\n"
    "               uint current = uint(textureLod(indexSampler, vec2(i, j), "
    "0.0));\n"
    "               if(current > 0U) {\n"
    "                   vec4 vertConf = textureLod(vertConfSampler, vec2(i, "
    "j), 0.0);\n"
    "                   vec4 colorTime = textureLod(colorTimeSampler, vec2(i, "
    "j), 0.0);\n"
    "                   if(colorTime.z < vColor.z && \n"
    "                      vertConf.w > confThreshold && \n"
    "                      vertConf.z > localPos.z && \n"
    "                      vertConf.z - localPos.z < 0.01 &&\n"
    "                      sqrt(dot(vertConf.xy - localPos.xy, vertConf.xy - "
    "localPos.xy)) < vNormRad.w * 1.4) {\n"
    "                       count++; }\n"
    ...
    "    if((vColor.w == -1.0 || ((float(time) - vColor.w) > 20.0 && "
    "vPosition.w < confThreshold))) {\n"
    "        test = 0;\n"
    "    }\n"
    ...
    "    //We don't deform vColor.z == time because they were fused with the "
    "updated pose already!\n"
    ...
    "        while(imax >= imin) {\n"
    "            imid = (imin + imax) / 2;\n"
    "            int nodeTime = int(textureLod(nodeSampler, vec2(((float(imid) "
    "* 16.0 + 15.0) / nodeCols) + (1.0 / (nodeCols * 2.0)), 0.5), 0.0).x);\n"
    ...
    "        int nodeMin = int(textureLod(nodeSampler, vec2(((float(imin) * "
    "16.0 + 15.0) / nodeCols) + (1.0 / (nodeCols * 2.0)), 0.5), 0.0).x);\n"
    "        int nodeMid = int(textureLod(nodeSampler, vec2(((float(imid) * "
    "16.0 + 15.0) / nodeCols) + (1.0 / (nodeCols * 2.0)), 0.5), 0.0).x);\n"
    "        int nodeMax = int(textureLod(nodeSampler, vec2(((float(imax) * "
    "16.0 + 15.0) / nodeCols) + (1.0 / (nodeCols * 2.0)), 0.5), 0.0).x);\n"
    ...
    "            vec3 position = vec3(textureLod(nodeSampler, vec2(((float(j) "
    "* 16.0) / nodeCols) + (1.0 / (nodeCols * 2.0)), 0.5), 0.0).x, \n"
    "                                 textureLod(nodeSampler, vec2(((float(j) "
    "* 16.0 + 1.0) / nodeCols) + (1.0 / (nodeCols * 2.0)), 0.5), 0.0).x, \n"
    "                                 textureLod(nodeSampler, vec2(((float(j) "
    "* 16.0 + 2.0) / nodeCols) + (1.0 / (nodeCols * 2.0)), 0.5), 0.0).x);\n"
    "            nearNodes[nearNodeIndex] = j;\n"
    "            nearDists[nearNodeIndex] = sqrt(dot(vPosition.xyz - position, "
    ...
    "        float weightSum = 0.0;\n"
    "        for(int j = 0; j < k; j++) {\n"
    "            vec3 position = vec3(textureLod(nodeSampler, "
    "vec2(((float(nearNodes[j]) * 16.0) / nodeCols) + (1.0 / (nodeCols * "
    "2.0)), 0.5), 0.0).x, \n"
    "                                 textureLod(nodeSampler, "
    "vec2(((float(nearNodes[j]) * 16.0 + 1.0) / nodeCols) + (1.0 / (nodeCols * "
    "2.0)), 0.5), 0.0).x, \n"
    "                                 textureLod(nodeSampler, "
    "vec2(((float(nearNodes[j]) * 16.0 + 2.0) / nodeCols) + (1.0 / (nodeCols * "
    "2.0)), 0.5), 0.0).x);\n"
    "            nodeWeights[j] = pow(1.0f - (sqrt(dot(vPosition.xyz - "
    "position, vPosition.xyz - position)) / dMax), 2.0);\n"
    ...
    "        for(int i = 0; i < k; i++) {\n"
    "            vec3 position = vec3(textureLod(nodeSampler, "
    "vec2(((float(nearNodes[i]) * 16.0) / nodeCols) + (1.0 / (nodeCols * "
    "2.0)), 0.5), 0.0).x, \n"
    "                                 textureLod(nodeSampler, "
    "vec2(((float(nearNodes[i]) * 16.0 + 1.0) / nodeCols) + (1.0 / (nodeCols * "
    "2.0)), 0.5), 0.0).x, \n"
    "                                 textureLod(nodeSampler, "
    "vec2(((float(nearNodes[i]) * 16.0 + 2.0) / nodeCols) + (1.0 / (nodeCols * "
    "2.0)), 0.5), 0.0).x);\n"
    "            vec3 column0 = vec3(textureLod(nodeSampler, "
    "vec2(((float(nearNodes[i]) * 16.0 + 3.0) / nodeCols) + (1.0 / (nodeCols * "
    "2.0)), 0.5), 0.0).x, \n"
    "                                textureLod(nodeSampler, "
    "vec2(((float(nearNodes[i]) * 16.0 + 4.0) / nodeCols) + (1.0 / (nodeCols * "
    "2.0)), 0.5), 0.0).x,\n"
    "                                textureLod(nodeSampler, "
    "vec2(((float(nearNodes[i]) * 16.0 + 5.0) / nodeCols) + (1.0 / (nodeCols * "
    "2.0)), 0.5), 0.0).x);\n"
    "            vec3 column1 = vec3(textureLod(nodeSampler, "
    "vec2(((float(nearNodes[i]) * 16.0 + 6.0) / nodeCols) + (1.0 / (nodeCols * "
    "2.0)), 0.5), 0.0).x, \n"
    "                                textureLod(nodeSampler, "
    "vec2(((float(nearNodes[i]) * 16.0 + 7.0) / nodeCols) + (1.0 / (nodeCols * "
    "2.0)), 0.5), 0.0).x,\n"
    "                                textureLod(nodeSampler, "
    "vec2(((float(nearNodes[i]) * 16.0 + 8.0) / nodeCols) + (1.0 / (nodeCols * "
    "2.0)), 0.5), 0.0).x);\n"
    "            vec3 column2 = vec3(textureLod(nodeSampler, "
    "vec2(((float(nearNodes[i]) * 16.0 + 9.0) / nodeCols) + (1.0 / (nodeCols * "
    "2.0)), 0.5), 0.0).x, \n"
    "                                textureLod(nodeSampler, "
    "vec2(((float(nearNodes[i]) * 16.0 + 10.0) / nodeCols) + (1.0 / (nodeCols "
    "* 2.0)), 0.5), 0.0).x,\n"
    "                                textureLod(nodeSampler, "
    "vec2(((float(nearNodes[i]) * 16.0 + 11.0) / nodeCols) + (1.0 / (nodeCols "
    "* 2.0)), 0.5), 0.0).x);\n"
    "            mat3 rotation = mat3(column0, column1, column2);\n"
    "            vec3 translation = vec3(textureLod(nodeSampler, "
    "vec2(((float(nearNodes[i]) * 16.0 + 12.0) / nodeCols) + (1.0 / (nodeCols "
    "* 2.0)), 0.5), 0.0).x, \n"
    "                                    textureLod(nodeSampler, "
    "vec2(((float(nearNodes[i]) * 16.0 + 13.0) / nodeCols) + (1.0 / (nodeCols "
    "* 2.0)), 0.5), 0.0).x,\n"
    "                                    textureLod(nodeSampler, "
    "vec2(((float(nearNodes[i]) * 16.0 + 14.0) / nodeCols) + (1.0 / (nodeCols "
    "* 2.0)), 0.5), 0.0).x);\n"
    "            newPos += nodeWeights[i] * (rotation * (vPosition.xyz - "
    "position) + position + translation);\n"
    "            newNorm += nodeWeights[i] * (transpose(inverse(rotation)) * "
    "vNormRad.xyz);\n"
    "        }\n"
    ...
    "            if(localPos.z > 0.0 && localPos.z < maxDepth && x > 0.0 && y "
    "> 0.0 && x < cols && y < rows) {\n"
    "                float currentDepth = float(textureLod(depthSampler, "
    "vec2(x / cols, y / rows), 0.0));\n"
    "                if(currentDepth > 0.0f && localPos.z < currentDepth + "
    "0.1f) {\n"
    "                    vColor.w = float(time);\n"
    "                }\n"
    "            }\n"
    ...
    "    }\n"
    "}\n";
static const std::tuple<std::string, std::string> copy_unstablevert_tuple =
    std::make_tuple("copy_unstablevert", copy_unstablevert_source);
...
static const char datafrag_source[] =
    ...
    "    }\n"
    "}\n";
static const std::tuple<std::string, std::string> datafrag_tuple =
    std::make_tuple("datafrag", datafrag_source);
...
static const char datageom_source[] =
    ...
    "void main() {\n"
    "    //Emit a vertex if either we have an update to store, or a new "
    "unstable vertex to store\n"
    "    if(updateId[0] > 0)  {\n"
    "        vPosition0 = vPosition[0];\n"
    "        vColor0 = vColor[0];\n"
    "        vNormRad0 = vNormRad[0];\n"
    ...
    "        \n"
    "        //This will be -10, -10 (offscreen) for new unstable vertices, so "
    "they don't show in the fragment shader\n"
    "        gl_Position = gl_in[0].gl_Position;\n"
    "        EmitVertex();\n"
    "        EndPrimitive(); \n"
    "    }\n"
    "}\n";
static const std::tuple<std::string, std::string> datageom_tuple =
    std::make_tuple("datageom", datageom_source);
...
static const char datavert_source[] =
    ...
    "    const float maxRadDist = 400.0; //sqrt((width * 0.5)^2 + (height * "
    "0.5)^2)\n"
    "    const float twoSigmaSquared = 0.72; //2*(0.6^2) from paper\n"
    "    vec2 pixelPosCentered = vec2(x, y) - cam.xy;\n"
    "    float radialDist = sqrt(dot(pixelPosCentered, pixelPosCentered)) / "
    "maxRadDist;\n"
    ...
    "vec3 getVertex(vec2 texCoord, float x, float y, vec4 cam, sampler2D "
    "depth) {\n"
    "    float z = float(textureLod(depth, texCoord, 0.0));\n"
    "    return vec3((x - cam.x) * z * cam.z, (y - cam.y) * z * cam.w, z);\n"
    "}\n"
    "//Cam is //cx, cy, 1 / fx, 1 / fy\n"
    "vec3 getNormal(vec3 vPosition, vec2 texCoord, float x, float y, vec4 cam, "
    "sampler2D depth) {\n"
    "    vec3 vPosition_xf = getVertex(vec2(texCoord.x + (1.0 / cols), "
    "texCoord.y), x + 1.0, y, cam, depth);\n"
    "    vec3 vPosition_xb = getVertex(vec2(texCoord.x - (1.0 / cols), "
    "texCoord.y), x - 1.0, y, cam, depth);\n"
    "    \n"
    "    vec3 vPosition_yf = getVertex(vec2(texCoord.x, texCoord.y + (1.0 / "
    "rows)), x, y + 1.0, cam, depth);\n"
    "    vec3 vPosition_yb = getVertex(vec2(texCoord.x, texCoord.y - (1.0 / "
    "rows)), x, y - 1.0, cam, depth);\n"
    "    \n"
    "    vec3 del_x = ((vPosition_xb + vPosition) / 2.0) - ((vPosition_xf + "
    "vPosition) / 2.0);\n"
    "    vec3 del_y = ((vPosition_yb + vPosition) / 2.0) - ((vPosition_yf + "
    "vPosition) / 2.0);\n"
    "    \n"
    ...
    "    return vec3((float(x) - cam.x) * z * cam.z, (float(y) - cam.y) * z * "
    "cam.w, z);\n"
    "}\n"
    "//Cam is //cx, cy, 1 / fx, 1 / fy\n"
    "vec3 getNormal(vec3 vPosition, vec2 texcoord, int x, int y, vec4 cam, "
    "usampler2D depth) {\n"
    "    vec3 vPosition_x = getVertex(vec2(texcoord.x + (1.0 / cols), "
    "texcoord.y), x + 1, y, cam, depth);\n"
    "    vec3 vPosition_y = getVertex(vec2(texcoord.x, texcoord.y + (1.0 / "
    "rows)), x, y + 1, cam, depth);\n"
    "    \n"
    ...
    "    float z = float(textureLod(depth, vec2(texCoord.x - (1.0 / cols), "
    "texCoord.y), 0.0));\n"
    "    if(z == 0.0)\n"
    "        return false;\n"
    "    z = float(textureLod(depth, vec2(texCoord.x, texCoord.y - (1.0 / "
    "rows)), 0.0));\n"
    "    if(z == 0.0)\n"
    "        return false;\n"
    "    z = float(textureLod(depth, vec2(texCoord.x + (1.0 / cols), "
    "texCoord.y), 0.0));\n"
    "    if(z == 0.0)\n"
    "        return false;\n"
    "    z = float(textureLod(depth, vec2(texCoord.x, texCoord.y + (1.0 / "
    ...
    "transformed to global coords\n"
    "    vec3 vNormLocal = getNormal(vPosition_f, texcoord.xy, x, y, cam, "
    "drfSampler);\n"
    "    vNormRad = vec4(mat3(pose) * vNormLocal, getRadius(vPosition_f.z, "
    "vNormLocal.z));\n"
    "    //Confidence\n"
    "    vPosition.w = confidence(x, y, weighting);\n"
    "    //Timestamp\n"
    "    //We update this in update.vert, because it's used to signal new "
    "points later on\n"
    "    vColor.w = 0.0;\n"
    "    updateId = 0;\n"
    "    uint best = 0U;\n"
    ...
    "        for(float i = texcoord.x - (scale * indexXStep * "
    "windowMultiplier); i < texcoord.x + (scale * indexXStep * "
    "windowMultiplier); i += indexXStep) {\n"
    "            for(float j = texcoord.y - (scale * indexYStep * "
    "windowMultiplier); j < texcoord.y + (scale * indexYStep * "
    "windowMultiplier); j += indexYStep) {\n"
    "               uint current = uint(textureLod(indexSampler, vec2(i, j), "
    "0.0));\n"
    "               if(current > 0U) {\n"
    "                   vec4 vertConf = textureLod(vertConfSampler, vec2(i, "
    "j), 0.0);\n"
    "                   if(abs((vertConf.z * lambda) - (vPosLocal.z * lambda)) "
    "< 0.05) {\n"
    "                       float dist = length(cross(ray, vertConf.xyz)) / "
    "length(ray);\n"
    "                       vec4 normRad = textureLod(normRadSampler, vec2(i, "
    "j), 0.0);\n"
    ...
    "        //should set gl_Position here to the 2D index for the updated "
    "vertex ID\n"
    "        gl_Position = vec4(float(int(intX) - (int(texDim) / 2)) / (texDim "
    "/ 2.0) + halfPixel, \n"
    "                           float(int(intY) - (int(texDim) / 2)) / (texDim "
    "/ 2.0) + halfPixel, \n"
    "                           0, \n"
    "                           1.0);\n"
    "    } else {\n"
    "        //Either don't render anything, or output a new unstable vertex "
    "offscreen\n"
    "        gl_Position = vec4(-10, -10, 0, 1);\n"
    "    }\n"
    "}\n";
static const std::tuple<std::string, std::string> datavert_tuple =
    std::make_tuple("datavert", datavert_source);
...
static const char depth_bilateralfrag_source[] =
    ...
    "      const float sigma_space2_inv_half = 0.024691358; // 0.5 / "
    "(sigma_space * sigma_space)\n"
    "      const float sigma_color2_inv_half = 0.000555556; // 0.5 / "
    ...
    "                float space2 = (float(x) - float(cx)) * (float(x) - "
    "float(cx)) + (float(y) - float(cy)) * (float(y) - float(cy));\n"
    "                float color2 = (float(value) - float(tmp)) * "
    "(float(value) - float(tmp));\n"
    "                float weight = exp(-(space2 * sigma_space2_inv_half + "
    ...
    "    }\n"
    "}\n";
static const std::tuple<std::string, std::string> depth_bilateralfrag_tuple =
    std::make_tuple("depth_bilateralfrag", depth_bilateralfrag_source);
...
static const char depth_metricfrag_source[] =
    ...
    "    }\n"
    "}\n";
static const std::tuple<std::string, std::string> depth_metricfrag_tuple =
    std::make_tuple("depth_metricfrag", depth_metricfrag_source);
...
static const char depth_normfrag_source[] =
    ...
    "        FragColor = 0.0;\n"
    "}\n";
static const std::tuple<std::string, std::string> depth_normfrag_tuple =
    std::make_tuple("depth_normfrag", depth_normfrag_source);
...
static const char depth_splatfrag_source[] =
    ...
    "    gl_FragDepth = (corrected_pos.z / (2.0 * maxDepth)) + 0.5f;\n"
    "}\n";
static const std::tuple<std::string, std::string> depth_splatfrag_tuple =
    std::make_tuple("depth_splatfrag", depth_splatfrag_source);

static const char draw_feedbackfrag_source[] = "#version 310 es\n"
                                               "precision highp float;\n"
                                               "precision highp int;\n"
                                               "in vec4 vColor;\n"
                                               "out vec4 FragColor;\n"
                                               ...
                                               "    FragColor = vColor;\n"
                                               "}\n";
static const std::tuple<std::string, std::string> draw_feedbackfrag_tuple =
    std::make_tuple("draw_feedbackfrag", draw_feedbackfrag_source);
...
static const char draw_feedbackvert_source[] =
    ...
    "        } else {\n"
    "            vColor = vec4((vec3(.5f, .5f, .5f) * abs(dot(normal.xyz, "
    "vec3(1.0, 1.0, 1.0)))) + vec3(0.1f, 0.1f, 0.1f), 1.0f);\n"
    "        }\n"
    ...
    "    }\n"
    "}\n";
static const std::tuple<std::string, std::string> draw_feedbackvert_tuple =
    std::make_tuple("draw_feedbackvert", draw_feedbackvert_source);
...
static const std::tuple<std::string, std::string>
    draw_global_surfacefrag_tuple = std::make_tuple(
        "draw_global_surfacefrag", draw_global_surfacefrag_source);
...
static const char draw_global_surfacegeom_source[] =
    ...
    "            float ratio = 2.0 * (vColor[0].z - minimum) / (maximum - "
    "minimum);\n"
    "            vColor0.x = max(0.0, (1.0 - ratio));\n"
    "            vColor0.y = max(0.0, (ratio - 1.0));\n"
    "            vColor0.z = 1.0f - vColor0.x - vColor0.y;\n"
    "            vColor0.xyz *= abs(dot(vNormRad[0].xyz, vec3(1.0, 1.0, 1.0))) "
    "+ vec3(0.1f, 0.1f, 0.1f);\n"
    "        } else  {\n"
    "            vColor0 = (vec3(.5f, .5f, .5f) * abs(dot(vNormRad[0].xyz, "
    "vec3(1.0, 1.0, 1.0)))) + vec3(0.1f, 0.1f, 0.1f);\n"
    "        }\n"
    "        if(float(drawWindow0[0]) == 1.0 && float(vTime[0]) - vColor[0].w "
    "> float(timeDelta0[0])) {\n"
    "            vColor0 *= 0.25;\n"
    "        }\n"
    "        vec3 x = normalize(vec3((vNormRad[0].y - vNormRad[0].z), "
    "-vNormRad[0].x, vNormRad[0].x)) * vNormRad[0].w * 1.41421356;\n"
    ...
    "}\n";
static const std::tuple<std::string, std::string>
    draw_global_surfacegeom_tuple = std::make_tuple(
        "draw_global_surfacegeom", draw_global_surfacegeom_source);
...
static const char draw_global_surfacevert_source[] =
    ...
    "}\n";
static const std::tuple<std::string, std::string>
    draw_global_surfacevert_tuple = std::make_tuple(
        "draw_global_surfacevert", draw_global_surfacevert_source);
...
static const char draw_global_surface_phongfrag_source[] =
    ...
    "}\n";
static const std::tuple<std::string, std::string>
    draw_global_surface_phongfrag_tuple = std::make_tuple(
        "draw_global_surface_phongfrag", draw_global_surface_phongfrag_source);

static const char emptyvert_source[] = "#version 310 es\n"
                                       "precision highp float;\n"
                                       "precision highp int;\n"
                                       "void main() {}\n";
static const std::tuple<std::string, std::string> emptyvert_tuple =
    std::make_tuple("emptyvert", emptyvert_source);
...
static const char fill_normalfrag_source[] =
    ...
    "        vec4 vPos = vec4(getVertex(texcoord, int(texcoord.x * cols), "
    "int(texcoord.y * rows), cam, rSampler), 1);\n"
    "        FragColor = vec4(getNormal(vPos.xyz, texcoord, int(texcoord.x * "
    "cols), int(texcoord.y * rows), cam, rSampler), 1);\n"
    "    } else {\n"
    "        FragColor = samp;\n"
    "    }\n"
    "}\n";
static const std::tuple<std::string, std::string> fill_normalfrag_tuple =
    std::make_tuple("fill_normalfrag", fill_normalfrag_source);
...
static const char fill_rgbfrag_source[] =
    ...
    "        FragColor = samp;\n"
    "}\n";
static const std::tuple<std::string, std::string> fill_rgbfrag_tuple =
    std::make_tuple("fill_rgbfrag", fill_rgbfrag_source);
...
static const char fill_vertexfrag_source[] =
    ...
    "        FragColor = vec4(getVertex(int(texcoord.x * cols), int(texcoord.y "
    "* rows), z), 1);\n"
    "    } else {\n"
    "        FragColor = samp;\n"
    "    }\n"
    "}\n";
static const std::tuple<std::string, std::string> fill_vertexfrag_tuple =
    std::make_tuple("fill_vertexfrag", fill_vertexfrag_source);
...
static const char fxaafrag_source[] =
    ...
    "    vec3 rgbA = 0.5 * (texture(tex, texcoord + dir * (1.0 / 3.0 - "
    "0.5)).xyz + texture(tex, texcoord + dir * (2.0 / 3.0 - 0.5)).xyz);\n"
    "    vec3 rgbB = rgbA * 0.5 + 0.25 * (texture(tex, texcoord + dir * "
    "-0.5).xyz + texture(tex, texcoord + dir * 0.5).xyz);\n"
    ...
    "}\n";
static const std::tuple<std::string, std::string> fxaafrag_tuple =
    std::make_tuple("fxaafrag", fxaafrag_source);
...
static const char index_mapfrag_source[] =
    ...
    "    FragColor = vertexId;\n"
    "}\n";
static const std::tuple<std::string, std::string> index_mapfrag_tuple =
    std::make_tuple("index_mapfrag", index_mapfrag_source);
...
static const char index_mapvert_source[] =
    ...
    "    float x = 0.0;\n"
    "    float y = 0.0;\n"
    "    if(vPosHome.z > maxDepth || vPosHome.z < 0.0 || float(time) - "
    "vColorTime.w > float(timeDelta)) {\n"
    "        x = -10.0;\n"
    "        y = -10.0;\n"
    "        vertexId = 0;\n"
    "    } else {\n"
    "        x = ((((cam.z * vPosHome.x) / vPosHome.z) + cam.x) - (cols * "
    "0.5)) / (cols * 0.5);\n"
    "        y = ((((cam.w * vPosHome.y) / vPosHome.z) + cam.y) - (rows * "
    "0.5)) / (rows * 0.5);\n"
    ...
    "}\n";
static const std::tuple<std::string, std::string> index_mapvert_tuple =
    std::make_tuple("index_mapvert", index_mapvert_source);
...
static const char init_unstablevert_source[] =
    ...
    "    vNormRad0 = vNormRad;\n"
    "}\n";
static const std::tuple<std::string, std::string> init_unstablevert_tuple =
    std::make_tuple("init_unstablevert", init_unstablevert_source);
...
static const char quadgeom_source[] =
    ...
    "    EndPrimitive(); \n"
    "}\n";
static const std::tuple<std::string, std::string> quadgeom_tuple =
    std::make_tuple("quadgeom", quadgeom_source);
...
static const char resizefrag_source[] =
    ...
    "}\n";
static const std::tuple<std::string, std::string> resizefrag_tuple =
    std::make_tuple("resizefrag", resizefrag_source);
...
static const char samplegeom_source[] =
    ...
    "    }\n"
    "}\n";
static const std::tuple<std::string, std::string> samplegeom_tuple =
    std::make_tuple("samplegeom", samplegeom_source);
...
static const char samplevert_source[] =
    ...
    "    id = gl_VertexID;    \n"
    "}\n";
static const std::tuple<std::string, std::string> samplevert_tuple =
    std::make_tuple("samplevert", samplevert_source);
...
static const char splatvert_source[] =
    ...
    "    return vec3(((((cam.z * p.x) / p.z) + cam.x) - (cols * 0.5)) / (cols "
    "* 0.5),\n"
    "                ((((cam.w * p.y) / p.z) + cam.y) - (rows * 0.5)) / (rows "
    "* 0.5),\n"
    "                p.z / maxDepth);\n"
    "}\n"
    ...
    "void main() {\n"
    "    vec4 vPosHome = t_inv * vec4(vPosition.xyz, 1.0);\n"
    "    if(vPosHome.z > maxDepth || vPosHome.z < 0.0 || vPosition.w < "
    "confThreshold || float(time) - vColor.w > float(timeDelta) || vColor.w > "
    "float(maxTime)) {\n"
    ...
    "        normRad = vec4(normalize(mat3(t_inv) * vNormRad.xyz), "
    "vNormRad.w);\n"
    "        vec3 x1 = normalize(vec3((normRad.y - normRad.z), -normRad.x, "
    "normRad.x)) * normRad.w * 1.41421356;\n"
    ...
    "        vec2 xs = vec2(min(proj1.x, min(proj2.x, min(proj3.x, proj4.x))), "
    "max(proj1.x, max(proj2.x, max(proj3.x, proj4.x))));\n"
    "        vec2 ys = vec2(min(proj1.y, min(proj2.y, min(proj3.y, proj4.y))), "
    "max(proj1.y, max(proj2.y, max(proj3.y, proj4.y))));\n"
    "        float xDiff = abs(xs.y - xs.x);\n"
    "        float yDiff = abs(ys.y - ys.x);\n"
    "        gl_PointSize = max(0.0, max(xDiff, yDiff));\n"
    "    }\n"
    "}\n";
static const std::tuple<std::string, std::string> splatvert_tuple =
    std::make_tuple("splatvert", splatvert_source);
...
static const char updatevert_source[] =
    ...
    "            vPosition0 = vec4(((c_k * v_k) + (a * v_g)) / (c_k + a), c_k "
    "+ a);\n"
    "            vec3 oldCol = decodeColor(vColor.x);\n"
    "            vec3 newCol = decodeColor(newColor.x);\n"
    "            vec3 avgColor = ((c_k * oldCol.xyz) + (a * newCol.xyz)) / "
    "(c_k + a);\n"
    "            vColor0 = vec4(encodeColor(avgColor), vColor.y, vColor.z, "
    "time);\n"
    ...
    "    }\n"
    "}\n";
static const std::tuple<std::string, std::string> updatevert_tuple =
    std::make_tuple("updatevert", updatevert_source);
...
static const char vertex_feedbackgeom_source[] =
    ...
    "    }\n"
    "}\n";
static const std::tuple<std::string, std::string> vertex_feedbackgeom_tuple =
    std::make_tuple("vertex_feedbackgeom", vertex_feedbackgeom_source);
...
static const char vertex_feedbackvert_source[] =
    ...
    "    vColor = textureLod(cSampler, texcoord.xy, 0.0);\n"
    "    vec3 vNormLocal = getNormal(vPosition.xyz, texcoord.xy, x, y, cam, "
    "gSampler);\n"
    ...
    "    vColor.w = float(time);\n"
    "}\n";
static const std::tuple<std::string, std::string> vertex_feedbackvert_tuple =
    std::make_tuple("vertex_feedbackvert", vertex_feedbackvert_source);
...
static const char visualise_texturesfrag_source[] =
    ...
    "    }\n"
    "}\n";
static const std::tuple<std::string, std::string> visualise_texturesfrag_tuple =
    std::make_tuple("visualise_texturesfrag", visualise_texturesfrag_source);
