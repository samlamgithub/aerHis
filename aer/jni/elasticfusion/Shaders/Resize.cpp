...
Resize::Resize(int srcWidth, int srcHeight, int destWidth, int destHeight)
    : imageTexture(destWidth, destHeight, GL_RGBA, GL_RGB, GL_UNSIGNED_BYTE,
                   false, true),
      vertexTexture(destWidth, destHeight, GL_RGBA32F, GL_LUMINANCE, GL_FLOAT,
                    false, true),
      timeTexture(destWidth, destHeight, GL_LUMINANCE16UI_EXT,
...
void Resize::time(GPUTexture *source, Img<unsigned short> &dest) {
  ...
  glDrawArrays(GL_POINTS, 0, 1);
  check_gl_errorResize();
  LOGI("MY elasitcfusion resize struct time 11");
  ...
  check_gl_errorResize();
  LOGI("MY elasitcfusion resize struct time 12");
