...
{LOGI("MY elasitcfusion CholeskyDecomp struct init ");}
...
//对于 ferns (local loop closure) 或者 ferns+pose (global loop closure) 设置相关联的控制点
void DeformationGraph::setPosesSeq(std::vector<unsigned long long int> * poseTimeMap, const std::vector<Eigen::Matrix4f> & poses)
{
    ...
    for(unsigned int i = 0; i < poses.size(); i++)
    {
        ...
        //对于每个点二分法查找时间节点最相近的控制点，因为控制点本来就是按照时间先后顺序排列的，所以可以使用二分法
        ...
        //对于时间最相近的控制点在其附近找控制点，按照控制点和 pose 的距离排序
        ...
        double weightSum = 0;
        //对于每个控制点计算更新的权值，并且 push 到一个 vector 中
...
//对 DeformationGraph 中的控制点做连接，因为参数在传进来时就已经按照时间的先后顺序排列
//这里把点和它周围 k 个 node 相连接
...
Eigen::VectorXd DeformationGraph::sparseResidual(const int maxRows)
{
    ...
    // 首先判断 constrain 对与 constrain 相连的的节点有没有影响，判断有没有影响通过判断约束点相关的 graph 的节点有没有连接，如果有连接说明 constrain 是对于 graph 是有作用的
    for(unsigned int l = 0; l < constraints.size(); l++)
    {
        const std::vector<VertexWeightMap> & weightMap = vertexMap.at(constraints.at(l).vertexId);
