...
RGBDOdometry::RGBDOdometry(int width, int height, float cx, float cy, float fx,
                           float fy, float distThresh, float angleThresh)
    : lastICPError(0), lastICPCount(width * height), lastRGBError(0),
      lastRGBCount(width * height), lastSO3Error(0),
      lastSO3Count(width * height),
      lastA(Eigen::Matrix<double, 6, 6, Eigen::RowMajor>::Zero()),
      lastb(Eigen::Matrix<double, 6, 1>::Zero()), sobelSize(3),
...
void RGBDOdometry::initICP(GPUTexture *predictedVertices,
  ...
  cudaGraphicsSubResourceGetMappedArray(&textPtr, predictedVertices->cudaRes, 0,
                                        0);
  cudaMemcpyFromArray(vmaps_tmp.ptr(), textPtr, 0, 0, vmaps_tmp.sizeBytes(),
                      cudaMemcpyDeviceToDevice);
  cudaGraphicsUnmapResources(1, &predictedVertices->cudaRes);

  cudaGraphicsMapResources(1, &predictedNormals->cudaRes);
  cudaGraphicsSubResourceGetMappedArray(&textPtr, predictedNormals->cudaRes, 0,
                                        0);
  cudaMemcpyFromArray(nmaps_tmp.ptr(), textPtr, 0, 0, nmaps_tmp.sizeBytes(),
                      cudaMemcpyDeviceToDevice);
...
void RGBDOdometry::initICPModel(GPUTexture *predictedVertices,
  ...
  Eigen::Matrix<float, 3, 3, Eigen::RowMajor> Rcam =
      modelPose.topLeftCorner(3, 3);
  ...
  for (int i = 0; i < NUM_PYRS; ++i) {
    tranformMaps(vmaps_g_prev_[i], nmaps_g_prev_[i], device_Rcam, device_tcam,
...
void RGBDOdometry::getIncrementalTransformation(
  ...
  if (rgb) {
    for (int i = 0; i < NUM_PYRS; i++) {
      // nextImage 当前帧图像用 sobel 在 GPU 中计算梯度
  ...
  if (so3) {
    ...
    for (int i = 0; i < 10; i++) {
      ...
      Eigen::Matrix<double, 3, 3, Eigen::RowMajor> homography =
          K * resultR * K.inverse();

      mat33 imageBasis;
      memcpy(&imageBasis.data[0], homography.cast<float>().eval().data(),
             sizeof(mat33));
      ...
      TICK("so3Step");
      so3Step(lastNextImage[pyramidLevel], nextImage[pyramidLevel], imageBasis,
              kinv, krlr, sumDataSO3, outDataSO3, jtj.data(), jtr.data(),
              &residual[0], so3StepThreads, so3StepBlocks);
      TOCK("so3Step");
      ...
      // Converged
      if (lastSO3Error < lastError && fabs(lastError - lastSO3Error) < 0.001) {
        break;
    ...
    for (int j = 0; j < iterations[i]; j++) {
      ...
      if (rgb) {
        ...
        computeRgbResidual(
            ...
            nextdIdx[i], nextdIdy[i], lastDepth[i], nextDepth[i], lastImage[i],
            nextImage[i], corresImg[i], sumResidualRGB, maxDepthDeltaRGB, kt,
            krkInv, sigma, rgbSize, rgbResThreads, rgbResBlocks);
      ...
      if (icp) {
        TICK("icpStep");
        icpStep(device_Rcurr, device_tcurr, vmap_curr, nmap_curr,
                device_Rprev_inv, device_tprev, intr(i), vmap_g_prev,
                nmap_g_prev, distThres_, angleThres_, sumDataSE3, outDataSE3,
                A_icp.data(), b_icp.data(), &residual[0], icpStepThreads,
                icpStepBlocks);
        TOCK("icpStep");
      }
      ...
      if (rgb) {
        TICK("rgbStep");
        rgbStep(corresImg[i], sigmaVal, pointClouds[i], intr(i).fx, intr(i).fy,
                nextdIdx[i], nextdIdy[i], sobelScale, sumDataSE3, outDataSE3,
                A_rgbd.data(), b_rgbd.data(), rgbStepThreads, rgbStepBlocks);
        TOCK("rgbStep");
      }
      ...
      Eigen::Matrix<double, 6, 6, Eigen::RowMajor> dA_rgbd =
          A_rgbd.cast<double>();
      Eigen::Matrix<double, 6, 6, Eigen::RowMajor> dA_icp =
          A_icp.cast<double>();
