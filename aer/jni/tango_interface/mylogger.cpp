...
void Mylogger::setCamWidthAndheight(int width, int height, double fx, double fy,
  ...
  myCx = cx;
  myCy = cy;
  LOGI("setCamWidthAndheight intrinsic: %d, %d, %f, %f, %f, %f , myImageSize: "
       "%d",
  ...
  //	    for(int i = 0; i < 10; i++)
  //	    {
  //	        uint8_t * newImage = (uint8_t *)calloc(imageSize * 3,
  //sizeof(uint8_t)); 	        imageBuffers[i] = std::pair<uint8_t *,
  //int64_t>(newImage, 0);
  //	    }
  //	    for(int i = 0; i < 10; i++) {
  for (int i = 0; i < 50; i++) {
...
void Mylogger::rgbdCallback(unsigned char *image,
  ...
  //    memcpy(frameBuffers[bufferIndex].first.first,
  //    reinterpret_cast<uint8_t*>(depth), myImageSize * 2); int
  //    rgbPixeldatacount = myImageSize * 3; LOGI("rgbPixeldatacount %d",
  //    rgbPixeldatacount); memcpy(frameBuffers[bufferIndex].first.second,
  //    reinterpret_cast<uint8_t*>(image), rgbPixeldatacount);
...
void Mylogger::writeData() {
  ...
  while (writing.getValueWait(1)) {
    ...
    if (err == TANGO_SUCCESS) {
      LOGI("CameraInterface: success get valid relative pose at %f time for "
           "color and depth cameras :%f , color > depth: %d",
           color_timestamp, depth_timestamp, color_timestamp > depth_timestamp);
    } else {
      LOGE("CameraInterface: Could not find a valid relative pose at %f time "
           "for color and depth cameras :%f ",
    ...
    LOGI("Logger 2 timestamp: %lld, depthSize : %lld,  imageSize: %lld ",
         (long long)(frameBuffers[bufferIndex].m_lastTimestamp),
