...
void MyElasticFusion::setCamWidthAndheight(int width, int height, double fx, double fy, double cx, double cy,
	int	maxVerCount) {
  ...
  depth_image_height = height;
	...
	myCx = cx;
	myCy = cy;
	LOGI("setCamWidthAndheight intrinsic: %d, %d, %f, %f, %f, %f , myImageSize: %d",
...
//	        unsigned char * newDepth = (unsigned char *)calloc(myImageSize * 2, sizeof(unsigned char));
	        ...
	        unsigned char * newImage = (unsigned char *)calloc(myImageSize * 3, sizeof(unsigned char));
...
void MyElasticFusion::rgbdCallback(unsigned char* image, TangoPointCloud* pointcloud_buffer, double color_timestamp, TangoPoseData pose) {
...
void MyElasticFusion::UpdateAndUpsampleDepth(const glm::mat4& color_t1_T_depth_t0, const float* render_point_cloud_buffer, std::vector<unsigned short> &depth_map_buffer_, int point_cloud_size) {
...
void MyElasticFusion::runEF() {
  ...
  // int file_start = 1;
  // int file_end = 782;
  vector<Eigen::Matrix4f> posesEigen;
  int32_t numFrames = 0;
  ...
  while(runningElasticFusion.getValueWait(1)) {
        ...
        //==============================================
          ...
          // uint32_t num_points = pointcloud_buffer->num_points;
        	...
        	LOGI("ElasticFusion depth_timestamp: %f , num_points:  %d", depth_timestamp, num_points);
        	double color_timestamp = frameBuffers[bufferIndex].colorTimeStamp;
        	 ...
        	 TangoErrorType err = TangoSupport_calculateRelativePose(
        	  ...
        	  if (err == TANGO_SUCCESS)  {
        		 LOGI("CameraInterface ElasticFusion: success get valid relative pose at %f time for color and depth cameras :%f , color > depth: %d",
        				 color_timestamp, depth_timestamp, color_timestamp > depth_timestamp);
        	  } else {
        	    LOGE("CameraInterface ElasticFusion: Could not find a valid relative pose at %f time for color and depth cameras :%f ", color_timestamp, depth_timestamp);
...
UpdateAndUpsampleDepth(color_image_t1_T_depth_image_t0, frameBuffers[bufferIndex].pointCloudPoints, depth_map_buffer_, num_points);
  ...
  unsigned long long int timeStampleForEF = frameBuffers[bufferIndex].m_lastTimestamp;
  ...
  Eigen::Vector3f trans((float)pose.translation[0], (float)pose.translation[1],
  ...
  currentPose.rightCols<1>() = trans;
    ...
    // float qw = (float)pose.orientation[3];
      ...
      // for (int file_ptr = file_start; file_ptr <= file_end; file_ptr = file_ptr +1) {
        ...
        // Eigen::Matrix4f * currentPose;//当前的位姿
          ...
          //  *currentPose = groundTruthOdometry->getTransformation(timestamp); }
           eFusion.processFrame(rgbImageForEF, depthForEF, timeStampleForEF, currentPose);
           delete currentPose;
      ...
      if (shouldSavePly.getValueWait()) {
          ...
          std::string plyFilename("/sdcard/ElasticFusionPly_" + current_date_time());
          ...
          if (fs == NULL) {
            LOGE("There was a problem opening the ply file:%s",plyFilename.c_str());
          }
