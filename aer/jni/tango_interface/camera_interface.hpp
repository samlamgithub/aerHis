...
typedef std::function<void (const DepthEvent& event)> RGBDCallBack;
...
//  static void setLogger(Mylogger* logger);
  // The gl initialisation are separated as they must be called from the gl thread
  static bool initialise(JNIEnv* env, jobject caller_activity, jobject asset_manager);
  static void destroy();
